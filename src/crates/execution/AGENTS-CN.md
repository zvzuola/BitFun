...
本层负责可复用的 agent、harness、stream、plugin host、typed-service 和 tool 执行原语。它不是完整 Agent Runtime SDK，也不是组装后的产品 runtime。由产品组装决定某个交付形态启用哪些 execution primitive、tool provider group、harness provider、adapter 和 service。
...
| `plugin-runtime-host` | 插件派发、生命周期、幂等、诊断和隔离的最小主机边界；不拥有 JS/TS 执行单元或具体生态适配器 | [AGENTS.md](plugin-runtime-host/AGENTS.md) |
...
- 可移植 execution 编排、agent lifecycle 契约、Plugin Runtime Host 边界逻辑、tool 契约、provider-neutral stream 契约和 execution facts 放到这里。
