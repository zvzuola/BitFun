...
function generatedHeader(language) {
  ...
  return [
    ...
    `${comment} Do not edit it by hand; edit src/shared/i18n/contract/locales.json instead.`,
    '',
  ].join('\n');
}

function generateWebLocaleContract(contract) {
...
function generateMobileLocaleContract(contract) {
...
function generateInstallerLocaleContract(contract) {
...
function generateCoreRustLocaleContract(contract) {
  const localeMap = getLocaleMap(contract);
...
function main() {
  ...
  for (const output of outputs) {
    const nextContent = output.generate(contract);
    if (checkOnly) {
