...
|---|---|---|---|---|---|
| 默认低摩擦 | PRD-01、PRD-02 | 快速路径轻量、体验视图层 | 配置化策略画像、项目画像 | P0 | 质量保障要求较低的探索性改动、无配置项目、文档改动任务回放；统计首次有用动作耗时和低风险任务完成率 |
| 内部策略不外露 | PRD-03、PRD-13 | 体验视图层、用户语言稳定 | 配置化策略画像 | P-1、P0 | 检查桌面、命令行、远程、PR 摘要是否只展示任务状态、原因和下一步，不展示内部枚举 |
...
| PR 就绪度 | PRD-07 | 证据按需生成视图、PR 场景显露 | PR 门禁、证据包 | P1、P2 | P1 可 shadow/advisory 生成就绪度摘要；P2 回放准备 PR、受保护分支、不可运行 CI 用例，验证就绪度摘要、未验证项、证据引用和非默认阻断 |
...
| 扩展接口 | PRD-16 | Plugin Runtime Host、Event Manifest、Tool ABI、插件诊断、插件效果候选、内核权威状态 | Plugin Runtime Host 与 OpenCode 兼容适配、质量数据面 | 产品架构 P0、SDLC P-1/P1 | 产品架构 P0 回放 OpenCode-compatible 插件来源、诊断和最小候选效果消费路径；SDLC P1 回放 availability/diagnostics、过期 epoch、重复 event 和候选效果丢弃 |
...
| 工具复写安全 | PRD-17 | 工具复写显式授权、复写表按项目执行域生效 | Plugin Runtime Host 与 OpenCode 兼容适配、安全边界 | P2 | 内置工具复写接口、同名工具候选、权限撤销、hash 变化、远程执行主机变化用例；验证复写不跨项目且仍经安全边界 |
| 关键用例体验 | PRD-18 | 体验视图层、页面/面板布局、跨入口语义一致 | 产品需求、配置化策略画像、质量数据面 | P0、P1、P2 | P0/P1 回放质量保障要求较低的探索性改动、文档改动、安全确认、远程容器和主动配置发现；P2 回放 PR 就绪和受管路径 |
...
| [implementation-plan.md](implementation-plan.md) | 阶段用户收益、必要技术前置、延期边界、验收条件和过程风险 | 不替代产品需求或模块接口 |
