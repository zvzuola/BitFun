...
| 扩展与插件 | 预留 Plugin Runtime Host contract、Event Manifest、Tool ABI 和候选契约，并对齐产品架构 P0 插件诊断事实 | availability/diagnostics 投影、过期候选和幂等校验 | 工具复写契约、UI descriptor 和主动配置信任 | P0 切片之外的 full plugin runtime 原型、生态评测 |
...
为准。实施时不能用 SDLC Harness 的扩展契约空实现替代 Desktop settings/command + CLI diagnostics 的 OpenCode-compatible plugin 真实消费闭环。
...
| P1 | 风险、远程和不可支持状态变得可理解，用户知道该补什么验证 | 弱提示、推荐检查、远程/沙箱降级、Plugin Runtime Host availability/diagnostics 投影、证据引用摘要 | 风险分类器、提示降噪、sandbox capability 事件、产品架构 P0 插件诊断事实消费 | 高风险解释清楚；低风险仍快速；远程和不可支持状态有替代路径 |
...
|---|---|---|---|---|---|
| P-1 | PRD、设计、最小事件信封、策略对象、安全边界、测试夹具、指标口径 | 技术跑道清晰；P0/P1 的收益、降级和验收方法可解释 | 契约过度设计导致后续实现被早期字段锁死；事件和策略对象未服务真实体验；测试夹具只覆盖理想路径 | 契约评审必须逐项映射 PRD、事件、用例和指标；新增字段要声明生产者、消费者、删除条件和进入用户路径的触发阶段 | 不设用户路径预算；限制设计期事件和指标数量，避免 P0 一开始背负遥测膨胀 |
...
| P1 | 风险提示、推荐检查、远程不可支持状态、沙箱降级说明、Plugin Runtime Host availability/diagnostics 投影、证据引用摘要 | 用户能理解风险和能力缺口，并知道补什么验证 | 风险提示泛化成噪音；推荐检查不可运行；远程缺口被本地验证掩盖；插件诊断投影被误认为 full runtime；证据引用缺少新鲜度 | 中/高风险用例必须说明原因、证据、建议和跳过后果；推荐检查要有不可运行分支；扩展候选不能写权威状态 | 风险分类和提示生成后台化；同类提示合并；扩展契约有 deadline、epoch、幂等键；证据引用只保存摘要和引用 |
...
| 插件和工具复写越权 | 外部插件、OpenCode 适配层或工具复写候选直接影响权威状态 | 执行链不可信，后续需要重构 Plugin Runtime Host、Tool ABI 和 Security Bridge 边界 | 早期只允许 Plugin Runtime Host availability/diagnostics 和候选契约进入安全链路；产品架构 P0 切片之外不扩张为 full runtime；候选不能写通过、失败、阻断、审计或授权状态 |
...
| 扩展契约 | Plugin Runtime Host availability/diagnostics、Event Manifest、Tool ABI、UI descriptor、插件效果候选、deadline、epoch 和幂等键的最小边界 | 服务产品架构 P0 的 Desktop/CLI 插件切片；非当前消费方字段必须写明删除条件 |
...
| 主动配置默认安全 | 发现 hook、plugin、MCP、自定义工具、工具复写和智能体规则，默认未信任且不执行 | 不执行未信任主动配置；不把产品架构 P0 的 Desktop/CLI 插件切片扩大成 full runtime |
...
| 扩展诊断可预留但不干扰 | Plugin Runtime Host availability/diagnostics、Event Manifest、Tool ABI、打点和候选契约；消费产品架构 P0 插件诊断事实 | 不执行未信任或 P0 切片之外的 OpenCode 插件/full runtime，不让候选写权威状态 |
