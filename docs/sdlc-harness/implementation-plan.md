...
| P2 | 准备或审查 PR 时，用户先获得目标正确的 Review，再获得可复制协作摘要和规则说明 | Review 目标证据、PR 就绪度、团队规则折叠说明、风险接受 | workspace/Git target evidence、只读证据状态、PR 视图、规则来源合成、风险接受记录 | 当前只闭合 workspace/Git range 目标正确；强制项只在配置或确定性风险下出现 |
...
| P2 | Review 目标证据、PR 就绪度、团队规则来源、路径规则、风险接受 | PR 或团队场景可准确审查目标并获得可复制摘要和规则说明；普通任务不被团队流程打断 | 审错 base/head；dirty workspace 污染；prepared diff 越界或超限；证据状态与建议混淆；团队规则冲突；PR 摘要过重 | workspace/range 目标与失效用例、有界 diff、聚合预算、越界、fail-closed 报告、规则来源和风险接受测试 | 目标证据使用明确状态；PR 视图按需生成；不自动 checkout；不创建默认强制检查 |
...
| 用户可见收益 | 必要实现 | 明确不做或延期 |
|---|---|---|
| Review 确实覆盖用户指定变更 | session-scoped target evidence：base/head、目标指纹、文件新旧路径与状态、完整度、workspace binding、最终 evidence status；prepared target 只用有界 `GetFileDiff`，`matching_clean` 时保留普通只读仓库上下文；旧入口保留既有 Git 能力 | 不增加 Reviewer Git 工具或逐调用全仓扫描；不让既有 Git 覆盖 prepared target 或扩大目标；不增加任意 shell；不自动 fetch/checkout/worktree；不建长期目标数据库或合成 diff refs |
...
- 当前工作区和显式 Git range 的 base/head/diff 与用户目标一致；显式文件/目录范围不扩大；clean checkout、删除/重命名、二进制、超限和远程缺口不会被解释为无变更或完整覆盖。provider 目标不在当前范围。
...
当前只承诺一个产品目标：让本地 workspace / Git range Review 诚实地绑定并消费目标证据。后续能力不得以 schema 预留、隐藏设置或兼容分支的名义提前进入。

#### 当前 PR：目标证据正确

状态：实现与验证中。合入后先观察目标解析失败率、limited evidence 比例、diff 预算耗尽率、Review 成功率和 token 变化，不直接进入 provider 接线。

| 项 | 计划 |
|---|---|
| 用户收益 | 显式 Git range 不再审错 revision；workspace 无法完整取证时不会伪装成完整覆盖，并得到诚实覆盖说明 |
| 必要实现 | 行为轻量的 target evidence contract；workspace/Git range 一次取证；准确文件状态与完整度；跨 Review 子会话的目标上下文传播；独立 evidence status；prepared target/report fail-close；opaque cursor 与父 Review turn 聚合预算；`matching_clean` 时保留普通 Read/Grep/Glob/LS 上下文 |
| 兼容 | 现有工作区 Review 保持默认路径；历史 manifest 无 target evidence 时明确走 legacy workspace fallback，不伪装成精确 range/PR |
| 风险前置 | 删除、重命名、二进制和超大文件；dirty workspace 污染；prepare 后工作区变化；remote Review 在有界 exact diff 前明确拒绝；文件名 prompt injection；Git 外部 diff/textconv 副作用 |
...
Provider/PR 入口、自动 Review、跨 Review finding、自动 checkout、Reviewer 命令执行、新结果动作和组织分析均不在当前计划内；需要时必须基于上线数据重新立项。
