...
> 用途：作为拆分后的入口文档。产品需求文档回答产品定位和体验要求，设计文档回答架构边界，实施计划回答阶段落地，子模块文档回答局部接口、schema 或状态视图。
...
| [architecture/evidence-pack.md](architecture/evidence-pack.md) | 证据包设计 | 证据包负责人、状态、生命周期、风险接受和 PR/审查/回放只读视图 |
...
- **扩展接口与首条生态切片分层**：先稳定 Plugin Runtime Host contract、binding、read/dispatch envelope、disabled stub、Event Manifest、Tool ABI、Permission/Effect 和插件诊断 / 候选效果接口；产品运行时 P0 以产品架构定义的 OpenCode-compatible 插件来源、诊断和最小候选效果消费路径为准。可写 JS/TS 插件运行时、未预算界面贡献、full plugin runtime、ACP permission bridge 和其他生态完整兼容后置到 P0+；这里的 P0+ 指产品架构 P0 后的独立产品决策阶段，不等同于 SDLC Harness P1。
...
4. 需要落地顺序时读实施计划。
5. 需要检查覆盖关系时读追踪矩阵。
6. 需要实现接口或结构化 schema 时再读配置化策略、安全边界、证据包、质量数据面（QDP）、风险分类和门禁等子模块。
