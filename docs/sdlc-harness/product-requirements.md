...
| 生态扩展预留 | 先稳定 Plugin Runtime Host contract、binding、envelope、disabled stub、Event Manifest、Tool ABI、Permission/Effect 和 UI descriptor；当前产品运行时 P0 需要一条 Desktop/CLI OpenCode-compatible plugin 真实消费切片，其他 full runtime 和生态完整兼容后续接入时必须在 BitFun 边界内生效 |
...
| 扩展契约 | Plugin Runtime Host contract、生命周期 envelope、Event Manifest、Tool ABI、Permission/Effect、UI descriptor 和插件效果候选 | 先提供稳定契约；产品架构 P0 切片之外的外部插件 full runtime 不作为默认能力 |
...
| 插件兼容与核心模型 | OpenCode 等生态后续通过适配器接入；BitFun 内部只依赖 Plugin Runtime Host、Event Manifest、Tool ABI、Permission/Effect 和 UI descriptor |
...
| PRD-16 | 扩展契约预留 | Plugin Runtime Host、Event Manifest、Tool ABI、Permission/Effect、UI descriptor 和插件效果候选有稳定契约，且未启用 adapter 不影响默认任务 |
...
为准：必须完成 Desktop settings/command + CLI diagnostics 的同一条 OpenCode-compatible plugin 垂直切片；SDLC Harness 不把这条真实消费路径替换成空合同或纯后续事项。

| 阶段 | 用户可见收益 | 后台前置和延期边界 |
|---|---|---|
| P0 | 低风险任务能快速完成，并看懂项目入口、安全范围、已验证/未验证项和下一步 | 只做轻量项目理解、基础安全决策和最小事件；完整证据包、图谱和团队门禁延期 |
| P1 | 风险、远程能力缺口、沙箱降级和推荐检查变得可理解 | 需要风险分类、提示降噪、sandbox capability 事件和扩展契约消费；不扩大到产品架构 P0 切片之外的外部插件 full runtime |
