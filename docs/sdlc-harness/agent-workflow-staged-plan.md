...
| 成本 | L2/L3 前显示计划独立检查/调用数、耗时倾向和收益，允许调整范围；不展示无法证明的 Token 估算 |
...
- L2/L3 扩大独立覆盖前展示单一 Review 方案确认，包括范围、计划 reviewer/调用数、耗时倾向和只读边界；不展示无法证明的 token 估算。
...
- Provider 入口收敛不是本轮承诺。候选边界仍是 PR adapter 保留 provider identity、base/head、远程 diff、CI/审批/mergeability 和评论发布，把 provider 事实映射为统一 Review 目标证据并消费结果投影，不拥有 reviewer 调度。只有上线指标证明现有 PR Review MiniApp 的平行路径仍造成高频重复决策，才重新立项；届时仍需保持平台权威事实和执行约束、既有变更就绪度 / PR 门禁、AI Review 建议三层分离。本轮不自动生成或发布 inline comment。Verify 的权威证据生产仍未在本阶段实现。

2026-07-10 合入后产品复盘：

- 统一入口、自适应强度、只读 Reviewer、独立 ReviewFixer 和同侧栏 follow-up 已形成可用基线，不再新增 Review 执行分支。
- 当前只承诺本地 workspace / Git range 目标证据收敛。该 PR 不新增 Review 执行分支、长期目标数据库、合成 diff 引用、增量缓存计划、Finding 生命周期、自动评论或结果动作。
- 后续首先观察目标解析失败率、limited evidence 比例、diff 预算耗尽率、Review 成功率和 token 变化。只有数据证明一个低耦合 provider 入口动作可显著解决高频决策障碍时才重新评估。
