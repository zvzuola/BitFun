...
GUI 只提供 `Review` 主动作，固定启动一个只读 reviewer。用户通过自然语言补充“更快”或“只看安全/性能/架构/前端”等目标时，由同一个模型调整调查重点；当前只有 `/review strict`、历史 `/DeepReview` alias 或内部显式 strict follow-up 进入严格主审路径，但也不自动形成多 reviewer，不新增可见档位菜单。
...
普通 Review 不根据规模或风险自动升级。系统只准备可靠的目标证据，并让一个 reviewer 在范围内决定调查深度：

- diff 很小或很大都保持一个 reviewer，风险标签只作为检查线索。
